...
class Registro(Frame):
    ...
    def create_wietgs(self):
        ...
        Label(self.frame2, text = 'Producto',fg='white', bg ='navy', font=('Rockwell',13,'bold')).grid(column=0,row=2, pady=15)
        Label(self.frame2, text = 'Insumos',fg='white', bg ='navy', font=('Rockwell',13,'bold')).grid(column=0,row=3, pady=15)
        Label(self.frame2, text = 'Producción_diaria', fg='white',bg ='navy', font=('Rockwell',13,'bold')).grid(column=0,row=4, pady=15)
        Label(self.frame2, text = 'reseta',fg='white', bg ='navy', font=('Rockwell',13,'bold')).grid(column=0,row=5, pady=15)

        Entry(self.frame2,textvariable=self.codigo , font=('Arial',12)).grid(column=1,row=1, padx =5)
        Entry(self.frame2,textvariable=self.Producto , font=('Arial',12)).grid(column=1,row=2)
        Entry(self.frame2,textvariable=self.Insumos , font=('Arial',12)).grid(column=1,row=3)
        Entry(self.frame2,textvariable=self.Producción_diaria , font=('Arial',12)).grid(column=1,row=4)
        Entry(self.frame2,textvariable=self.reseta , font=('Arial',12)).grid(column=1,row=5)
        ...
        Button(self.frame4,command = self.buscar_producto, text='BUSCAR POR PRODUCTO', font=('Arial',8,'bold'), bg='orange').grid(columnspan=2,column = 1, row=2)
        ...
        self.tabla['columns'] = ('Producto', 'Insumos', 'Produccion_diaria', 'Resetas')

        self.tabla.column('#0', minwidth=100, width=120, anchor='center')
        self.tabla.column('Producto', minwidth=100, width=130 , anchor='center')
        self.tabla.column('Insumos', minwidth=100, width=120, anchor='center' )
        self.tabla.column('Produccion_diaria', minwidth=100, width=120 , anchor='center')
        self.tabla.column('resetas', minwidth=100, width=105, anchor='center')

        self.tabla.heading('#0', text='Codigo', anchor ='center')
        self.tabla.heading('Nombre', text='Producto', anchor ='center')
        self.tabla.heading('Modelo', text='Insumos', anchor ='center')
        self.tabla.heading('Precio', text='Produccion_diaria', anchor ='center')
        self.tabla.heading('Cantidad', text='Resetas', anchor ='center')
    ...
    def agregar_datos(self):
        ...
        datos = (Producto, Insumos, Produccion_diaria, resetas)
        if codigo and producto and insumos and producion_diaria and resetas !='':        
            self.tabla.insert('',0, text = codigo, values=datos)
            self.base_datos.inserta_producto(codigo, producto, insumos, Produccion_diarias, resetas)
    ...
    def eliminar_fila(self):
        ...
        if len(fila) !=0:        
            ...
            nombre = ("'"+ str(self.producto_borrar) + "'")       
            self.base_datos.elimina_productos(productos)
